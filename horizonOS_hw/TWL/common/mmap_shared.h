...
#ifndef NN_HW_TWL_COMMON_MMAP_SHARED_H_
#define NN_HW_TWL_COMMON_MMAP_SHARED_H_
#ifdef  __cplusplus
extern  "C" {
#endif
/*---------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------*
    TWL CARD ROM HEADER
 *----------------------------------------------------------------------*/
#define HW_TWL_CARD_ROM_HEADER_BUF              (HW_TWL_MAIN_MEM + 0x00ffc000)
 ...
 *----------------------------------------------------------------------*/
#define HW_SYSM_VER_INFO_CONTENT_ID                 (HW_TWL_MAIN_MEM + 0x00ffd7b0 ) // 8bytesのcontentID, NULL終端あり
#define HW_SYSM_VER_INFO_CONTENT_LAST_INITIAL_CODE  (HW_TWL_MAIN_MEM + 0x00ffd7b9 )
...
/*----------------------------------------------------------------------*
    TitleID list for system use
 *----------------------------------------------------------------------*/
#define HW_TWL_OS_TITLE_ID_LIST                 (HW_TWL_MAIN_MEM + 0x00ffd800 ) // ユーザー用TitleIDリスト
#define HW_TWL_OS_TITLE_ID_LIST_SIZE            0x400

/*----------------------------------------------------------------------*
    FS PARAMETER from SystemMenu
 *----------------------------------------------------------------------*/
#define HW_TWL_FS_MOUNT_INFO_BUF                (HW_TWL_MAIN_MEM + 0x00ffdc00 ) // マウント情報（サイズは0x3c0確保。11個までマウント指定可能。11*84+1=0x39d）
#define HW_TWL_FS_BOOT_SRL_PATH_BUF             (HW_TWL_MAIN_MEM + 0x00ffdfc0 ) // 起動SRLのパス

/*----------------------------------------------------------------------*
    MEMORY MAP of temporary area for sheltering each extended rom header
 *----------------------------------------------------------------------*/
#define HW_TWL_ROM_HEADER_BUF                   (HW_TWL_MAIN_MEM + 0x00ffe000)
 ...
 *----------------------------------------------------------------------*/
#define HW_NTR_CARD_ROM_HEADER              (HW_TWL_MAIN_MEM + 0x00fffa80)
#define HW_NTR_CARD_ROM_HEADER_SIZE         0x160   // 352 bytes
#define HW_NTR_CARD_ROM_HEADER_END          (HW_NTR_CARD_ROM_HEADER + HW_NTR_CARD_ROM_HEADER_SIZE)

/*----------------------------------------------------------------------*
    MEMORY MAP of SYSTEM SHARED AREA (1K bytes)
 *----------------------------------------------------------------------*/
#define HW_NTR_ROM_HEADER_BUF               (HW_TWL_MAIN_MEM + 0x00fffe00)      // ＲＯＭ内登録エリアデータ・バッファ (END-0x200)
#define HW_NTR_ROM_HEADER_BUF_SIZE          0x160   // 352 bytes
#define HW_NTR_ROM_HEADER_BUF_END           (HW_TWL_MAIN_MEM + 0x00ffff60)      //                                    (END-0x0a0)

/*----------------------------------------------------------------------*
    MEMORY MAP of SYSTEM SHARED AREA (1K bytes)
 *----------------------------------------------------------------------*/
#define HW_NTR_BOOT_CHECK_INFO_BUF          (HW_TWL_MAIN_MEM + 0x00fffc00)      // Boot check info (END-0x400)
#define HW_NTR_BOOT_CHECK_INFO_BUF_END      (HW_TWL_MAIN_MEM + 0x00fffc20)      //                 (END-0x3e0)
#define HW_NTR_BOOT_CHECK_INFO_BUF_SIZE     (HW_NTR_BOOT_CHECK_INFO_BUF_END - HW_NTR_BOOT_CHECK_INFO_BUF)

#define HW_NTR_NVRAM_USER_INFO              (HW_TWL_MAIN_MEM + 0x00fffc80)      // NVRAM user info           (END-0x380)
#define HW_NTR_NVRAM_USER_INFO_END          (HW_TWL_MAIN_MEM + 0x00fffd68)      //                           (END-0x298)
#define HW_NTR_NVRAM_USER_INFO_SIZE         (HW_NTR_NVRAM_USER_INFO_END - HW_NTR_NVRAM_USER_INFO)

#define HW_TWL_HW_SECURE_INFO               (HW_TWL_MAIN_MEM + 0x00fffd68)      // 本体セキュア情報          (END-0x298)
#define HW_TWL_HW_SECURE_INFO_END           (HW_TWL_MAIN_MEM + 0x00fffd80)      //                           (END-0x280)
#define HW_TWL_HW_SECURE_INFO_SIZE          (HW_TWL_HW_SECURE_INFO_END - HW_TWL_HW_SECURE_INFO)
...
#endif
#endif  /* NN_HW_TWL_COMMON_MMAP_SHARED_H_ */
