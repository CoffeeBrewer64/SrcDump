...
u32 nOpcode = *(u32*)pNesCpuPC;
#else
u8 nOpcode = *pNesCpuPC;
#endif
#else
u8 nOpcode = *g_pNesCpuPC;
#endif

LB_NES_CPU_OPCODE:
#if TRL_CPU_OPCODE_REG_FAST_OPCODE
switch(nOpcode&0x00FF)
#else
switch(nOpcode)
#endif
{
...
case 0x10:	//BPL disp
	if(!(P & NES_CPU_NFLAG))
	{
		...
		SET_PC(nTemp16);
#if TRL_HVC_CPU_BRANCH_SHORTTIME
		SET_TICK(2);
#else
		if((nAddr^PC)&0x100)
			SET_TICK(4);
...
#else
	g_nNesCpuEventTicks = g_nNesCpuTicks;
#endif
	break;
case 0xD8:	//CLD
	P &= ~NES_CPU_DFLAG;
