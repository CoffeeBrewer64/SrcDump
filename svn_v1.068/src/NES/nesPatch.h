...
case 2: // Fixvalue
	...
	case 7:
		//Mach Rider Save
		memcpy(g_pNesSRAM,&g_pNesWRAM[0x700],0x80);
		g_pNesEmuBuf->nSaveUpdate |= 1;
		break;
	case 8:
		//Mach Rider Load
		memcpy(&g_pNesWRAM[0x700],g_pNesSRAM,0x80);
		break;
	case 9:
...
case 6: // FixMemory  rangelzhang
		{
			...
			if(pPatch->sData.sA.nType==1){  //video memory write
				...
				for(int i=0;i<pPatch->sData.sA.nFixcode;i++)
								nesPpuMemDirectWrite8(pPatch->sData.sA.nIndex + i, pPatch->pFixcode[i]);
			}
	...
	switch(pPatch->sData.sA.nIndex){
			case 1:			//LOAD
				*pReg = pPatch->pFixcode[0];				
				break;
			case 2:          //shift right
				for(int i=0;i<pPatch->pFixcode[0];i++)
				{
					*pReg >>= 1;
				}
				break;
...
default:
	g_pNesEmuBuf->sPatchActive[g_pNesEmuBuf->nPatchActive].Clear();
	g_pNesEmuBuf->sPatchActive[g_pNesEmuBuf->nPatchActive].nIndex = nID;
	g_pNesEmuBuf->sPatchActive[g_pNesEmuBuf->nPatchActive].nMode = pPatch->sData.sA.nMode;
