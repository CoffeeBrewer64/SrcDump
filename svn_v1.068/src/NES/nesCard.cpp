...
inline void nesCardRegPRGBank8(u32 nBank, u32 nStartBank)
{
...
inline void nesCardRegPRGBank16(u32 nBank, u32 nStartBank)
{
	nBank &= g_nNesCardPRGBankMask;
	g_pNesPRGBank[nStartBank] = g_pNesPRGROM + nBank * NES_CARD_BANKSIZE_PRG;
...
inline void nesCardRegPRGBank32(u32 nBank)
{
	nBank &= g_nNesCardPRGBankMask;
...
inline void nesCardRegCHRBank1(u32 nBank, u32 nStartBank)
{
...
inline void nesCardRegCHRBank2(u32 nBank, u32 nStartBank)
{
	g_bNesGrpSPRChange |= 2;
	nBank &= g_nNesCardCHRBankMask;
	g_pNesCHRBank[nStartBank] = g_pNesCHRROM + nBank * NES_CARD_BANKSIZE_CHR;
...
inline void nesCardRegCHRBank4(u32 nBank, u32 nStartBank)
{
	g_bNesGrpSPRChange |= 2;
	nBank &= g_nNesCardCHRBankMask;
	g_pNesCHRBank[nStartBank] = g_pNesCHRROM + nBank * NES_CARD_BANKSIZE_CHR;
...
inline void nesCardRegCHRBank8(u32 nBank)
{
	g_bNesGrpSPRChange |= 2;
	nBank &= g_nNesCardCHRBankMask;
...
inline void nesCardRegSRAMBank(u32 nBank)
{
	g_pNesSRAMBank = g_pNesSRAM + nBank * NES_CARD_BANKSIZE_SRAM;
}

#if TRL_DEBUG
extern s32 g_nTrlEmuMapper;
extern s32 g_nTrlEmuFdiskSide;
#endif
...
} g_sNesCardRegMapper1;

/** @brief Update PRG Bank - MMC-1 */
inline void nesCardRegUpdatePRGBankMapper1()
{
...
void nesCardRegMapper5Write(u16 nAddr, u8 nValue)
{
	...
	case 0x5106:	// Name Table Fill-Mode Tile Number
		...
		memset(g_pNesCardFILLRAMMapper5, g_sNesCardRegMapper5.nFillTileNumber, 0x3C0);
		break;
	case 0x5107:	// Name Table Fill-Mode Palette Number
		nValue &= 0x3;
		g_sNesCardRegMapper5.nFillPalNumber = nValue + (nValue<<2) + (nValue<<4) + (nValue<<6);
		memset(&g_pNesCardFILLRAMMapper5[0x3C0], g_sNesCardRegMapper5.nFillPalNumber, 0x40);
		break;
...
//		NN_LOG("L:%d  IRQCount:%d\n", g_nNesGrpLines, g_sNesCardRegMapper5.nIRQCount);
		break;
	case 0x5204:	// Vertical IRQ Control/Status (R/W)
		g_nNesCardIRQEnable = (nValue & 0x80) ? NES_CARD_IRQ_SCANLINE : 0;
//		NN_LOG("L:%d  IRQEnable:%d\n", g_nNesGrpLines, g_nNesCardIRQEnable);
		break;
...
u8 nesCardRegMapper5Read(u16 nAddr)
{
	...
	case 0x5203:	// Vertical IRQ Counter
		return g_sNesCardRegMapper5.nIRQCount;
	case 0x5204:	// Vertical IRQ Control/Status (R/W)
		{
...
#else
	if(g_sNesCardRegMapper5.nIRQCount==(g_nNesGrpLines+1)){
		g_nNesCpuIRQFlag = 1;
	}
#endif
}
...
} g_sNesCardRegMapper13;

/** @brief Update Name Table - SUNSOFT4 */
inline void nesCardRegUpdateNameTable()
{
...
void nesCardRegMapper25(u16 nAddr, u8 nValue)
{
	...
	case 0x9:
	case 0xA:
		nAddr = ((nAddrX << 12) | nAddrY) - 0x9000;
		nAddr = (nAddrX - 0x9)*4 + nAddrY;
		nesSndRegWriteCard(nAddr, nValue);
		break;
	case 0xB:	// Name Table Control
		...
		{
			//VRC
			nesSndRegWriteCard(nAddrY + 0x8, nValue);
		}
		else if(nAddrY == 3)
		{
...
} g_sNesCardRegMapper36;
u8*	g_pNesCardVRAMMapper36 = NULL;	///< VRAM

/** @brief Switch 1K CHR Bank - NAMCOT-106 */
inline void nesCardRegCHRBankMapper36(u32 nBank, u32 nStartBank)
{
	g_bNesGrpSPRChange |= 2;
...
struct nesSCardRegMapper51
{
	u32		nCHRBank;			///< CHR Bank
} g_sNesCardRegMapper51;
...
{
	nesSCardMapper100SaveHeader* pHeader = (nesSCardMapper100SaveHeader*)g_pNesSRAM;
	u8* pData = g_pNesSRAM + sizeof(nesSCardMapper100SaveHeader)*NES_CARD_DISKSYSTEM_SAVEMAX;

	if(bIO)	// Write
	{
		for(u32 n=0; n<g_pNesROMHeader->nSideNumber; ++n)
			g_pNesSRAM[NES_CARD_BANKSIZE_SRAM-NES_CARD_DISKSYSTEM_SIDEMAX+n] = g_nNesCardMapper100File[n];
		for(u32 n=0; n<NES_CARD_DISKSYSTEM_SAVEMAX; ++n, ++pHeader)
		{
			if(!pHeader->nAddr)
				return;
	...
	else	// Read
	{
		...
		// Init File List
		memset(g_sNesCardMapper100FileInfo, 0, sizeof(nesSCardMapper100FileInfo)*NES_CARD_DISKSYSTEM_SIDEMAX*NES_CARD_DISKSYSTEM_FILEMAX);
		for(u32 n=0; n<g_pNesROMHeader->nSideNumber; ++n)
		{
			u8* pDisk = g_pNesCardMapper100DiskHeader + NES_CARD_DISKSYSTEM_SIDESIZE*n;
			nesSCardMapper100FileInfo* pInfo = g_sNesCardMapper100FileInfo[n];
			u8 nBoot = pDisk[0x19];
			u32 nFile = 0;
			for(u32 nPos=56+g_pNesROMHeader->nCRC; nPos<NES_CARD_DISKSYSTEM_SIDESIZE;)
			{
				switch(pDisk[nPos])
				{
...
#if TRL_HVC_FDSDISK_SWITCHDISK_OLD_METHOD
/** @brief Disk System Update */
void nesCardDiskSystemUpdate()
{
	u16 nDisk = nesMemRead16(*(u16*)g_pNesWRAM + 6);
	g_sNesCardRegMapper100.nSide = (nDisk >> 8) * 2 + (nDisk & 0xFF);
...
inline u8* nesCardDiskSystemGetPointer(u16 nArea, u16 nAddr)
{
	if(!nArea)	// WRAM
	{
		switch(nAddr >> 13)
...
inline u16 nesCardDiskSystemGetParameter(u16 nOffset)
{
...
{
	u8* pDiskID = nesCardDiskSystemGetPointer(0, nesCardDiskSystemGetParameter(1));
	if(*pDiskID == 0xFF){
		...
		u8 *thisDiskID;
		for(int i=0;i<g_pNesROMHeader->nSideNumber;i++)
		{
			...
			if(memcmp(pDiskID, thisDiskID, 10)==0)
				break;
		}
...
#if TRL_DEBUG
	g_nTrlEmuFdiskSide=g_sNesCardRegMapper100.nSide;
#endif
	g_pNesCardMapper100Disk = g_pNesCardMapper100DiskHeader + g_sNesCardRegMapper100.nSide * 0x10000;
//	NN_LOG("Read: Disk%d Side%d\n", pDiskID[0x07], pDiskID[0x06]);

	u8 nFound = 0;
	u8* pLoadList = nesCardDiskSystemGetPointer(0, nesCardDiskSystemGetParameter(3));
	if(*pLoadList == 0xFF)	// Load Boot Files
	{
//		NN_LOG("Load Boot Files\n");
		nesSCardMapper100FileInfo* pInfo = g_sNesCardMapper100FileInfo[g_sNesCardRegMapper100.nSide];
		for(u32 n=0; n<g_nNesCardMapper100File[g_sNesCardRegMapper100.nSide]; ++n, ++pInfo)
		{
			if(pInfo->bBoot)
			{
				trlMemCopy(nesCardDiskSystemGetPointer(pInfo->nArea, pInfo->nTarget), g_pNesCardMapper100Disk + pInfo->nAddr, pInfo->nSize);
				nFound++;
//				NN_LOG("%d: %d 0x%04X->0x%04X (0x%04X)\n", nFound, pInfo->nArea, pInfo->nAddr, pInfo->nTarget, pInfo->nSize);
			}
		}
	}
	else
	{
		...
		for(u32 n=0; n<g_nNesCardMapper100File[g_sNesCardRegMapper100.nSide]; ++n, ++pInfo)
		{
			for(u32 m=0; m<NES_CARD_DISKSYSTEM_LOADMAX; ++m)
			{
				if(pLoadList[m] == 0xFF)
					break;
				if(pLoadList[m] == g_pNesCardMapper100Disk[pInfo->nHeader])
				{
					...
					nFound++;
					break;
//					NN_LOG("%d: ID=0x%02X AREA=%d 0x%04X->0x%04X (0x%04X)\n", nFound, pLoadList[m], pInfo->nArea, pInfo->nAddr, pInfo->nTarget, pInfo->nSize);
				}
			}
		}
	}
...
void nesCardDiskSystemWrite()
{
	...
	u8* pFileInfo = nesCardDiskSystemGetPointer(0, nesCardDiskSystemGetParameter(3));
	u16 nSrcAddr = *(u16*)(&pFileInfo[14]);
	u16 nSrcArea = pFileInfo[16];
	nesSCardMapper100FileInfo* pInfo = g_sNesCardMapper100FileInfo[g_sNesCardRegMapper100.nSide];
...
//		NN_LOG("Append: Disk%d Side%d\n", pDiskID[0x07], pDiskID[0x06]);
	...
	else if(g_nNesCpuRegA>=g_nNesCardMapper100File[g_sNesCardRegMapper100.nSide])
	{
		...
		nesSCardMapper100FileInfo* pLastInfo = pInfo-1;
...
//	NN_LOG("%d 0x%04X->0x%04X (0x%04X) ID:0x%02X->0x%02X\n", nSrcArea, nSrcAddr, pInfo->nAddr, pInfo->nSize, pInfo->nID, pFileInfo[0]);
	...
	// Update Save
	u32 nSize = 0x11 + g_pNesROMHeader->nCRC;
	u32 nAddress = g_sNesCardRegMapper100.nSide * 0x10000 + pInfo->nAddr - nSize;
	u32 nLength = sizeof(nesSCardMapper100SaveHeader) * NES_CARD_DISKSYSTEM_SAVEMAX;
	nesSCardMapper100SaveHeader* pHeader = (nesSCardMapper100SaveHeader*)g_pNesSRAM;
	nSize += pInfo->nSize;
	for(u32 n=0; n<NES_CARD_DISKSYSTEM_SAVEMAX; ++n, ++pHeader)
	{
...
inline void nesCardDiskSystemReloadRom(void)
{
...
void nesCardRegMapper100Write(u16 nAddr, u8 nValue)
{
			...
			if(g_sNesCardRegMapper100.nIOPtr < NES_CARD_DISKSYSTEM_DISKSIZE)
			{
				if(g_sNesCardRegMapper100.nIOSkip)
				{
					if(!g_sNesCardRegMapper100.nIOGAP)
					{
						...
						case 1:
							{
								u32 nAddress = g_sNesCardRegMapper100.nSide * 0x10000 + g_sNesCardRegMapper100.nIOPtr;
								u32 nSize = sizeof(nesSCardMapper100SaveHeader) * NES_CARD_DISKSYSTEM_SAVEMAX;
								nesSCardMapper100SaveHeader* pHeader = (nesSCardMapper100SaveHeader*)g_pNesSRAM;
								for(u32 n=0; n<NES_CARD_DISKSYSTEM_SAVEMAX; ++n, ++pHeader)
								{
									if(nAddress)
	...
	case 0x4025:	// Disk Control Register
		...
		// Bit6   GAP Control
#if 0		// No CRC Disk
		if(!(nValue & 0x40) && g_sNesCardRegMapper100.nIOGAP && (g_sNesCardRegMapper100.nIOCRC==0))
		{
			g_sNesCardRegMapper100.nIOPtr -= 2;
			if(g_sNesCardRegMapper100.nIOPtr < 0)
				g_sNesCardRegMapper100.nIOPtr = 0;
		}
#endif
...
u8 nesCardRegMapper100Read(u16 nAddr)
{
	...
	case 0x4030:	// Disk Status Register 0
		{
			u8 nData = (u8)g_nNesCpuIRQFlag | 0x80;
			g_nNesCpuIRQFlag = 0;
			return nData;
		}
	case 0x4031:	// Disk Data Read Register
		{
			u8 nData = g_pNesCardMapper100Disk[g_sNesCardRegMapper100.nIOPtr];
			if(g_sNesCardRegMapper100.nIOPtr < NES_CARD_DISKSYSTEM_DISKSIZE)
...
void nesCardInit()
{
	...
	{
		s32 nSRAMSize = g_pNesROMHeader->nSRAMBanks * NES_CARD_BANKSIZE_SRAM;
		g_pNesSRAM = (u8*)trlMemAlloc(nSRAMSize);
		g_pNesSRAMBank = g_pNesSRAM;
		g_nNesCardSRAMEnable = 1;
		memset(g_pNesSRAM, 0, nSRAMSize);
		// Backup Data
		if(g_pNesROMHeader->nBattery)
		{
		...
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(g_pNesSRAM, nSRAMSize, 1, 0);
	}
	...
	u32 nRenderMode = NES_GRP_BG_NORMAL;
	...
	case 1:		// MMC1 - PRG/32K/16K, VROM/8K/4K, NT
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper1, sizeof(nesSCardRegMapper1), 1, 0);
		nesCardRegister = nesCardRegMapper1;
		break;
	case 2:		// MMC2 - PRG/24K/8K, VROM/4K, NT, LATCH
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper2, sizeof(nesSCardRegMapper2), 1, 0);
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_nNesCardLatch, 8, 1, 0);
	...
	case 3:		// MMC3 - PRG/8K, VROM/2K/1K, VT, SRAM, IRQ
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_nNesCardSRAMEnable, 4, 1, 0);
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_nNesCardIRQEnable, 4, 1, 0);
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper3, sizeof(nesSCardRegMapper3), 1, 0);
		g_nNesCardIRQEnable = 0;
		nesCardRegister = nesCardRegMapper3;
		nesCardUpdateIRQ = nesCardUpdateIRQMapper3;
		break;
	case 4:		// MMC4 - PRG/16K, VROM/4K, NT, LATCH
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper4, sizeof(nesSCardRegMapper4), 1, 0);
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_nNesCardLatch, 8, 1, 0);
	...
	case 5:		// MMC5 - BANKING, IRQ, SOUND, VIDEO, MULTIPLY, etc.
		...
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(g_pNesCardEXRAMMapper5, 0x400, 1, 0);
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_nNesCardIRQEnable, 4, 1, 0);
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper5, sizeof(nesSCardRegMapper5), 1, 0);
		g_nNesCardIRQEnable = 0;
	...
	case 6:		// UNROM - PRG/16K
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper6, sizeof(nesSCardRegMapper6), 1, 0);
		nesCardRegister = nesCardRegMapper6;
		break;
	case 7:		// CNROM - VROM/8K
		g_nNesCardSRAMEnable = 2;
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper7, sizeof(nesSCardRegMapper7), 1, 0);
		nesCardRegister = nesCardRegMapper7;
		g_nPPUCHRRAM=0;
		break;
	case 8:		// GNROM - PRG/32K, VROM/8K
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper8, sizeof(nesSCardRegMapper8), 1, 0);
		nesCardRegister = nesCardRegMapper8;
		break;
	case 9:		// RARE - PRG/32K, Name Table Select
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper9, sizeof(nesSCardRegMapper9), 1, 0);
		nesCardRegister = nesCardRegMapper9;
		break;
	case 10:	// SUNSOFT1 - VROM/4K
		g_nNesCardSRAMEnable = 2;
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper10, sizeof(nesSCardRegMapper10), 1, 0);
		nesCardRegister = nesCardRegMapper10;
		break;
	case 11:	// SUNSOFT2 - PRG/16K, VROM/8K, NT
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper11, sizeof(nesSCardRegMapper11), 1, 0);
		nesCardRegister = nesCardRegMapper11;
		break;
	case 12:	// SUNSOFT3 - PRG/16K, VROM/2K, IRQ
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_nNesCardIRQTicks, 4, 1, 0);
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_nNesCardIRQEnable, 4, 1, 0);
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper12, sizeof(nesSCardRegMapper12), 1, 0);
	...
	case 13:	// SUNSOFT4 - PRG/16K, VROM/2K, NT-VROM
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper13, sizeof(nesSCardRegMapper13), 1, 0);
		nesCardRegister = nesCardRegMapper13;
		break;
	case 14:	// SUNSOFT5 - PRG/8K, VROM/1K, NT ctrl, SRAM, IRQ
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_nNesCardIRQTicks, 4, 1, 0);
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_nNesCardIRQEnable, 4, 1, 0);
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper14, sizeof(nesSCardRegMapper14), 1, 0);
		g_nNesCardIRQTicks = 0;
		g_nNesCardIRQEnable = 0;
	...
	case 15:	// BANDAI - PRG/16K, VROM/8K, NT
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper15, sizeof(nesSCardRegMapper15), 1, 0);
		nesCardRegister = nesCardRegMapper15;
		break;
	case 16:	// VRC1 - PRG/8K, VROM/4K, NT
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper16, sizeof(nesSCardRegMapper16), 1, 0);
		nesCardRegister = nesCardRegMapper16;
		break;
	case 17:	// VRC2A - PRG/8K, VROM/1K, NT
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper17, sizeof(nesSCardRegMapper17), 1, 0);
		nesCardRegister = nesCardRegMapper17;
		break;
	case 18:	// VRC2B - PRG/8K, VROM/1K, NT, IRQ
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_nNesCardIRQEnable, 4, 1, 0);
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper18, sizeof(nesSCardRegMapper18), 1, 0);
		g_nNesCardIRQEnable = 0;
	...
	case 19:	// VRC3 - PRG/16K, IRQ
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_nNesCardIRQTicks, 4, 1, 0);
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_nNesCardIRQEnable, 4, 1, 0);
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper19, sizeof(nesSCardRegMapper19), 1, 0);
		g_nNesCardIRQTicks = 0;
		g_nNesCardIRQEnable = 0;
	...
	case 24:	// VRC4E - PRG/8K, VROM/1K, NT, IRQ
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_nNesCardSRAMEnable, 4, 1, 0);
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_nNesCardIRQEnable, 4, 1, 0);
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper20, sizeof(nesSCardRegMapper20), 1, 0);
		g_nNesCardIRQEnable = 0;
	...
	case 26:	// VRC6B - PRG/16K/8K, VROM/1K, NT, IRQ, SOUND
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_nNesCardSRAMEnable, 4, 1, 0);
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_nNesCardIRQEnable, 4, 1, 0);
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper25, sizeof(nesSCardRegMapper25), 1, 0);
		g_nNesCardIRQEnable = 0;
	...
	case 27:	// VRC7A/B - PRG/16K/8K, VROM/1K, NT, IRQ, SOUND
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_nNesCardIRQEnable, 4, 1, 0);
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper27, sizeof(nesSCardRegMapper27), 1, 0);
		g_nNesCardIRQEnable = 0;
	...
	case 28:	// JALECO-1 - VROM/8K
		g_nNesCardSRAMEnable = 2;
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper28, sizeof(nesSCardRegMapper28), 1, 0);
		nesCardRegister = nesCardRegMapper28;
		break;
	case 29:	// JALECO-2 - PRG-LO, VROM/8K
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper29, sizeof(nesSCardRegMapper29), 1, 0);
		nesCardRegister = nesCardRegMapper29;
		break;
	case 30:	// JALECO-3 - PRG-HI, VROM/8K
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper30, sizeof(nesSCardRegMapper30), 1, 0);
		nesCardRegister = nesCardRegMapper30;
		break;
	case 31:	// JALECO-4 - PRG/32K, VROM/8K
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper31, sizeof(nesSCardRegMapper31), 1, 0);
		g_nNesCardSRAMEnable = 2;
	...
	case 32:	// JALECO-SS8806 - PRG/8K, VROM/1K, NT, IRQ, EXT
		...
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_nNesCardIRQTicks, 4, 1, 0);
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_nNesCardIRQEnable, 4, 1, 0);
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper32, sizeof(nesSCardRegMapper32), 1, 0);
		g_nNesCardIRQTicks = 0;
		g_nNesCardIRQEnable = 0;
	...
	case 34:	// 74161B - PRG/16K
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper33, sizeof(nesSCardRegMapper33), 1, 0);
		nesCardRegister = nesCardRegMapper33;
		break;
	case 35:	// NAMCOT - PRG/8K, VROM/2K/1K
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper35, sizeof(nesSCardRegMapper35), 1, 0);
	...
	case 36:	// NAMCOT-106 - PRG/8K, VROM/1K/VRAM, IRQ, SOUND
		...
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(g_pNesCardVRAMMapper36, 0x8000, 1, 0);
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper36, sizeof(nesSCardRegMapper36), 1, 0);
	...
	case 37:	// NAMCO-109 - PRG/8K, VROM/2K
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper37, sizeof(nesSCardRegMapper37), 1, 0);
		nesCardRegister = nesCardRegMapper37;
		break;
	case 38:	// NAMCO-118
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper38, sizeof(nesSCardRegMapper38), 1, 0);
		nesCardRegister = nesCardRegMapper38;
		break;
	case 39:	// MMC-3+TLS - PRG/8K, VROM/2K/1K, VT, SRAM, IRQ
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_nNesCardIRQEnable, 4, 1, 0);
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper39, sizeof(nesSCardRegMapper39), 1, 0);
		g_nNesCardIRQEnable = 0;
	...
	case 40:	// TAITO-TC0190 - PRG/8K, VROM/1K/2K, NT
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper40, sizeof(nesSCardRegMapper40), 1, 0);
		nesCardRegister = nesCardRegMapper40;
		break;
	case 41:	// TAITO-TC0350 - PRG/8K, VROM/1K/2K, NT, IRQ
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_nNesCardIRQEnable, 4, 1, 0);
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper41, sizeof(nesSCardRegMapper41), 1, 0);
		g_nNesCardIRQEnable = 0;
	...
	case 42:	// TAITO-TC190V - PRG/8K, VROM/1K/2K, NT, IRQ
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_nNesCardIRQEnable, 4, 1, 0);
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper42, sizeof(nesSCardRegMapper42), 1, 0);
		g_nNesCardIRQEnable = 0;
		nesCardRegister = nesCardRegMapper42;
		nesCardUpdateIRQ = nesCardUpdateIRQMapper42;
		break;
	case 43:	// TAITO-X-005 - PRG/8K, VROM/2K/1K, NT
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper43, sizeof(nesSCardRegMapper43), 1, 0);
		g_nNesCardSRAMEnable = 2;
		nesCardRegister = nesCardRegMapper43;
		break;
	case 44:	// TAITO-X1-17 - PRG/8K, VROM/2K/1K
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper44, sizeof(nesSCardRegMapper44), 1, 0);
		g_nNesCardSRAMEnable = 2;
		nesCardRegister = nesCardRegMapper44;
		break;
	case 45:	// IREM-1 - PRG/32K, VROM/2K, VRAM 6K+2K
		g_pNesCardVRAMMapper45 = (u8*)trlMemAlloc(0x2000);
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(g_pNesCardVRAMMapper45, 0x2000, 1, 0);
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper45, sizeof(nesSCardRegMapper45), 1, 0);
	...
	case 46:	// IREM-2 - PRG HI
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper46, sizeof(nesSCardRegMapper46), 1, 0);
		nesCardRegister = nesCardRegMapper46;
		break;
	case 47:	// IREM-74HC161 - PRG/16K, VROM/8K
	case 48:	// IREM-74HC32 - PRG/16K, VROM/8K
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper47, sizeof(nesSCardRegMapper47), 1, 0);
		nesCardRegister = nesCardRegMapper47;
		break;
	case 49:	// IREM-G-101 - PRG/8K, VROM/1K, NT
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper49, sizeof(nesSCardRegMapper49), 1, 0);
		nesCardRegister = nesCardRegMapper49;
		break;
	case 50:	// IREM-H-3001 - PRG/8K, VROM/1K, NT, IRQ
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_nNesCardIRQEnable, 4, 1, 0);
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper50, sizeof(nesSCardRegMapper50), 1, 0);
...
//add by yuans:
	case 51:	// Mapper 185: VROM-disable
		g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper51, sizeof(nesSCardRegMapper51), 1, 0);
	...
	case 100:	// Disk System - PRG RAM, BIOS, DISK, IRQ, SOUND
		{
			...
			memset(g_pNesSRAM, 0, NES_CARD_BANKSIZE_SRAM);
			g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_nNesCardIRQTicks, 4, 1, 0);
			g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_nNesCardIRQEnable, 4, 1, 0);
			g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_nNesCardIRQDiskTicks, 4, 1, 0);
			g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_nNesCardIRQDiskEnable, 4, 1, 0);
			g_sNesCardSaveData[g_nNesCardSaveData++].SetData(&g_sNesCardRegMapper100, sizeof(nesSCardRegMapper100), 1, 0);
			g_sNesCardSaveData[g_nNesCardSaveData++].SetData(g_pNesSRAM, NES_CARD_BANKSIZE_SRAM, 1, 0);
...
void nesCardReset()
{
	...
	g_nNesCardIRQDiskEnable = 0;
#if TRL_DEBUG
	g_nTrlEmuMapper = g_pNesROMHeader->nMapper;
#endif
	switch(g_pNesROMHeader->nMapper)
	{
	case 0:		// NROM - (No Mapper)
	...
	case 1:		// MMC1 - PRG/32K/16K, VROM/8K/4K, NT
		...
		nesCardRegPRGBank16(g_pNesROMHeader->nPRGBanks - 2, 2);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper1, 0, sizeof(nesSCardRegMapper1));
	...
	case 2:		// MMC2 - PRG/24K/8K, VROM/4K, NT, LATCH
		...
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper2, 0, sizeof(nesSCardRegMapper2));
		break;
	case 3:		// MMC3 - PRG/8K, VROM/2K/1K, VT, SRAM, IRQ
		nesCardRegPRGBank16(0, 0);
		nesCardRegPRGBank16(g_pNesROMHeader->nPRGBanks - 2, 2);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper3, 0, sizeof(nesSCardRegMapper3));
		break;
	case 4:		// MMC4 - PRG/16K, VROM/4K, NT, LATCH
		nesCardRegPRGBank16(0, 0);
		nesCardRegPRGBank16(g_pNesROMHeader->nPRGBanks - 2, 2);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper4, 0, sizeof(nesSCardRegMapper4));
		break;
	case 5:		// MMC5 - BANKING, IRQ, SOUND, VIDEO, MULTIPLY, etc.
		memset(g_pNesCardFILLRAMMapper5, 0, 0x400);
		memset(g_pNesCardEXRAMMapper5, 0, 0x400);
		g_pNesSRAMBank = g_pNesSRAM;
		memset(&g_sNesCardRegMapper5, 0, sizeof(nesSCardRegMapper5));
	...
	case 6:		// UNROM - PRG/16K
		...
		nesCardRegPRGBank16(g_pNesROMHeader->nPRGBanks - 2, 2);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper6, 0, sizeof(nesSCardRegMapper6));
		break;
	case 7:		// CNROM - VROM/8K
		nesCardRegPRGBank32(0);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper7, 0, sizeof(nesSCardRegMapper7));
		g_nPPUCHRRAM=0;
		break;
	case 8:		// GNROM - PRG/32K, VROM/8K
		nesCardRegPRGBank32(0);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper8, 0, sizeof(nesSCardRegMapper8));
		break;
	case 9:		// RARE - PRG/32K, Name Table Select
		nesCardRegPRGBank32(0);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper9, 0, sizeof(nesSCardRegMapper9));
		break;
	case 10:	// SUNSOFT1 - VROM/4K
		nesCardRegPRGBank32(0);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper10, 0, sizeof(nesSCardRegMapper10));
		break;
	case 11:	// SUNSOFT2 - PRG/16K, VROM/8K, NT
		nesCardRegPRGBank16(0, 0);
		nesCardRegPRGBank16(g_pNesROMHeader->nPRGBanks - 2, 2);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper11, 0, sizeof(nesSCardRegMapper11));
		break;
	case 12:	// SUNSOFT3 - PRG/16K, VROM/2K, IRQ
		nesCardRegPRGBank16(0, 0);
		nesCardRegPRGBank16(g_pNesROMHeader->nPRGBanks - 2, 2);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper12, 0, sizeof(nesSCardRegMapper12));
		break;
	case 13:	// SUNSOFT4 - PRG/16K, VROM/2K, NT-VROM
		nesCardRegPRGBank16(0, 0);
		nesCardRegPRGBank16(g_pNesROMHeader->nPRGBanks - 2, 2);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper13, 0, sizeof(nesSCardRegMapper13));
		break;
	case 14:	// SUNSOFT5 - PRG/8K, VROM/1K, NT ctrl, SRAM, IRQ
		for(u32 n=0; n<3; ++n)
		...
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper14, 0, sizeof(nesSCardRegMapper14));
		break;
	case 15:	// BANDAI - PRG/16K, VROM/8K, NT
		nesCardRegPRGBank16(0, 0);
		nesCardRegPRGBank16(g_pNesROMHeader->nPRGBanks - 2, 2);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper15, 0, sizeof(nesSCardRegMapper15));
		break;
	case 16:	// VRC1 - PRG/8K, VROM/4K, NT
		nesCardRegPRGBank16(0, 0);
		...
		nesCardRegPRGBank8(g_pNesROMHeader->nPRGBanks - 1, 3);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper16, 0, sizeof(nesSCardRegMapper16));
		break;
	case 17:	// VRC2A - PRG/8K, VROM/1K, NT
		nesCardRegPRGBank16(0, 0);
		nesCardRegPRGBank16(g_pNesROMHeader->nPRGBanks - 2, 2);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper17, 0, sizeof(nesSCardRegMapper17));
		break;
	case 18:	// VRC2B - PRG/8K, VROM/1K, NT, IRQ
		nesCardRegPRGBank16(0, 0);
		nesCardRegPRGBank16(g_pNesROMHeader->nPRGBanks - 2, 2);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper18, 0, sizeof(nesSCardRegMapper18));
		break;
	case 19:	// VRC3 - PRG/16K, IRQ
		nesCardRegPRGBank16(0, 0);
		nesCardRegPRGBank16(g_pNesROMHeader->nPRGBanks - 2, 2);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper19, 0, sizeof(nesSCardRegMapper19));
		break;
	case 20:	// VRC4A - PRG/8K, VROM/1K, NT, IRQ
	case 21:	// VRC4B - PRG/8K, VROM/1K, NT, IRQ
	case 22:	// VRC4C - PRG/8K, VROM/1K, NT, IRQ
	...
	case 24:	// VRC4E - PRG/8K, VROM/1K, NT, IRQ
		...
		nesCardRegPRGBank16(g_pNesROMHeader->nPRGBanks - 2, 2);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper20, 0, sizeof(nesSCardRegMapper20));
	...
	case 26:	// VRC6B - PRG/16K/8K, VROM/1K, NT, IRQ, SOUND
		...
		nesCardRegPRGBank8(g_pNesROMHeader->nPRGBanks - 1, 3);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper25, 0, sizeof(nesSCardRegMapper25));
	...
	case 27:	// VRC7A/B - PRG/16K/8K, VROM/1K, NT, IRQ, SOUND
		...
		nesCardRegPRGBank8(g_pNesROMHeader->nPRGBanks - 1, 3);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper27, 0, sizeof(nesSCardRegMapper27));
		break;
	case 28:	// JALECO-1 - VROM/8K
		nesCardRegPRGBank16(0, 0);
		nesCardRegPRGBank16(g_pNesROMHeader->nPRGBanks - 2, 2);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper28, 0, sizeof(nesSCardRegMapper28));
		break;
	case 29:	// JALECO-2 - PRG-LO, VROM/8K
		nesCardRegPRGBank16(0, 0);
		nesCardRegPRGBank16(g_pNesROMHeader->nPRGBanks - 2, 2);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper29, 0, sizeof(nesSCardRegMapper29));
		break;
	case 30:	// JALECO-3 - PRG-HI, VROM/8K
		nesCardRegPRGBank16(0, 0);
		nesCardRegPRGBank16(g_pNesROMHeader->nPRGBanks - 2, 2);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper30, 0, sizeof(nesSCardRegMapper30));
		g_sNesCardRegMapper30.nPRGBank = g_pNesROMHeader->nPRGBanks - 2;
		break;
	case 31:	// JALECO-4 - PRG/32K, VROM/8K
		nesCardRegPRGBank32(0);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper31, 0, sizeof(nesSCardRegMapper31));
		break;
	case 32:	// JALECO-SS8806 - PRG/8K, VROM/1K, NT, IRQ, EXT
		nesCardRegPRGBank16(0, 0);
		nesCardRegPRGBank8(0, 2);
		nesCardRegPRGBank8(g_pNesROMHeader->nPRGBanks - 1, 3);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper32, 0, sizeof(nesSCardRegMapper32));
		g_sNesCardRegMapper32.nIRQWidth = 0xFFFF;
		break;
	case 33:	// 74161A - PRG/16K
	case 34:	// 74161B - PRG/16K
		...
		nesCardRegPRGBank16(g_pNesROMHeader->nPRGBanks - 2, 2);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper33, 0, sizeof(nesSCardRegMapper33));
	...
	case 35:	// NAMCOT - PRG/8K, VROM/2K/1K
		...
		nesCardRegPRGBank16(g_pNesROMHeader->nPRGBanks - 2, 2);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper35, 0, sizeof(nesSCardRegMapper35));
		break;
	case 36:	// NAMCOT-106 - PRG/8K, VROM/1K/VRAM, IRQ, SOUND
		memset(g_pNesCardVRAMMapper36, 0, 0x8000);
		nesCardRegPRGBank16(0, 0);
		nesCardRegPRGBank16(g_pNesROMHeader->nPRGBanks - 2, 2);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper36, 0, sizeof(nesSCardRegMapper36));
		break;
	case 37:	// NAMCO-109 - PRG/8K, VROM/2K
		nesCardRegPRGBank16(0, 0);
		nesCardRegPRGBank16(g_pNesROMHeader->nPRGBanks - 2, 2);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper37, 0, sizeof(nesSCardRegMapper37));
		break;
	case 38:	// NAMCO-118
		nesCardRegPRGBank16(0, 0);
		nesCardRegPRGBank16(g_pNesROMHeader->nPRGBanks - 2, 2);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper38, 0, sizeof(nesSCardRegMapper38));
		break;
	case 39:	// MMC-3+TLS - PRG/8K, VROM/2K/1K, VT, SRAM, IRQ
		nesCardRegPRGBank16(0, 0);
		nesCardRegPRGBank16(g_pNesROMHeader->nPRGBanks - 2, 2);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper39, 0, sizeof(nesSCardRegMapper39));
		break;
	case 40:	// TAITO-TC0190 - PRG/8K, VROM/1K/2K, NT
		nesCardRegPRGBank16(0, 0);
		nesCardRegPRGBank16(g_pNesROMHeader->nPRGBanks - 2, 2);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper40, 0, sizeof(nesSCardRegMapper40));
		break;
	case 41:	// TAITO-TC0350 - PRG/8K, VROM/1K/2K, NT, IRQ
		nesCardRegPRGBank16(0, 0);
		nesCardRegPRGBank16(g_pNesROMHeader->nPRGBanks - 2, 2);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper41, 0, sizeof(nesSCardRegMapper41));
		break;
	case 42:	// TAITO-TC190V - PRG/8K, VROM/1K/2K, NT, IRQ
		nesCardRegPRGBank16(0, 0);
		nesCardRegPRGBank16(g_pNesROMHeader->nPRGBanks - 2, 2);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper42, 0, sizeof(nesSCardRegMapper42));
		break;
	case 43:	// TAITO-X-005 - PRG/8K, VROM/2K/1K, NT
		nesCardRegPRGBank16(0, 0);
		nesCardRegPRGBank8(0, 2);
		nesCardRegPRGBank8(g_pNesROMHeader->nPRGBanks - 1, 3);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper43, 0, sizeof(nesSCardRegMapper43));
		break;
	case 44:	// TAITO-X1-17 - PRG/8K, VROM/2K/1K
		nesCardRegPRGBank16(0, 0);
		nesCardRegPRGBank8(0, 2);
		nesCardRegPRGBank8(g_pNesROMHeader->nPRGBanks - 1, 3);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper44, 0, sizeof(nesSCardRegMapper44));
		break;
	case 45:	// IREM-1 - PRG/32K, VROM/2K, VRAM 6K+2K
		memset(g_pNesCardVRAMMapper45, 0, 0x2000);
		nesCardRegPRGBank32(0);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper45, 0, sizeof(nesSCardRegMapper45));
		break;
	case 46:	// IREM-2 - PRG HI
		nesCardRegPRGBank16(g_pNesROMHeader->nPRGBanks - 2, 0);
		nesCardRegPRGBank16(0, 2);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper46, 0, sizeof(nesSCardRegMapper46));
		break;
	case 47:	// IREM-74HC161 - PRG/16K, VROM/8K
	case 48:	// IREM-74HC32 - PRG/16K, VROM/8K
		nesCardRegPRGBank16(0, 0);
		nesCardRegPRGBank16(g_pNesROMHeader->nPRGBanks - 2, 2);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper47, 0, sizeof(nesSCardRegMapper47));
	...
	case 49:	// IREM-G-101 - PRG/8K, VROM/1K, NT
		...
		nesCardRegPRGBank16(g_pNesROMHeader->nPRGBanks - 2, 2);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper49, 0, sizeof(nesSCardRegMapper49));
		break;
	case 50:	// IREM-H-3001 - PRG/8K, VROM/1K, NT, IRQ
		nesCardRegPRGBank16(0, 0);
		nesCardRegPRGBank16(g_pNesROMHeader->nPRGBanks - 2, 2);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper50, 0, sizeof(nesSCardRegMapper50));
		break;
		//add by yuans
	case 51:	// Mapper 185: VROM-disable
		nesCardRegPRGBank32(0);
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper51, 0, sizeof(nesSCardRegMapper51));
		break;
		//end of add
	case 100:	// Disk System - PRG RAM, BIOS, DISK, IRQ, SOUND
		...
		nesCardRegCHRBank8(0);
		memset(&g_sNesCardRegMapper100, 0, sizeof(nesSCardRegMapper100));
...
void nesCardResume()
{
	...
	case 5:		// MMC5 - BANKING, IRQ, SOUND, VIDEO, MULTIPLY, etc.
		...
		for(u32 n=0; n<(12 - g_sNesCardRegMapper5.nEXRAMMode*4); ++n)
