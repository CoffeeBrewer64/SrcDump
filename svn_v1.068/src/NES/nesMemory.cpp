...
void nesMemInit()
{
	...
	// Save Data
	g_nNesMemSaveData = 0;
	g_sNesMemSaveData[g_nNesMemSaveData++].SetData(g_pNesWRAM, 0x800, 1, 0);
	g_sNesMemSaveData[g_nNesMemSaveData++].SetData(g_pNesVRAM, 0x800, 1, 0);
	g_sNesMemSaveData[g_nNesMemSaveData++].SetData(g_pNesSPRRAM, 0x100, 1, 0);
	g_sNesMemSaveData[g_nNesMemSaveData++].SetData(g_pNesPALRAM, 0x20, 1, 0);
	if(g_pNesROMHeader->nCHRBanks == 0){
		//modify by yuans: change 8K --> 16K
		g_sNesMemSaveData[g_nNesMemSaveData++].SetData(g_pNesCHRROM, 0x2000, 1, 0);
		//end of modify	
	}
	if(g_pNesROMHeader->nPRGBanks == 0)
		g_sNesMemSaveData[g_nNesMemSaveData++].SetData(g_pNesPRGROM, 0x8000, 1, 0);
...
{
	{
		u8 wramvalue = g_pNesEmuBuf->funcConfigData("NES", "WRAMInitValue", 0);		
		memset(g_pNesWRAM, wramvalue, 0x800);
	}
	memset(g_pNesVRAM, 0, 0x800);
	memset(g_pNesSPRRAM, 0, 0x100);
	memset(g_pNesPALRAM, 0, 0x20);
	if(!g_pNesROMHeader->nPRGBanks)
		memset(g_pNesPRGROM, 0, 0x8000);
	if(!g_pNesROMHeader->nCHRBanks)
		//modify by yuans: change 8K --> 16K
		memset(g_pNesCHRROM, 0, 0x2000);
		//endof modify
}
