...
inline s32 __GetSamplesPerPitch(u32 pitch)
{
	if(pitch==0)
		return 0;
...
inline s8 __GetWave_Pulse(s32 type,s32 counter)
{
	return g_pWavePulse[type][counter];
}

inline u32 __GetWave_Pulse2(s32 type,u32 *counter,u32 pitch)
{	
	u8 total = 0;
#if TRL_SOUND_NES_FAST
	for(u32 i=0 ; i < 2 ;i++)
	{
		*counter = *counter + pitch;
		total += g_pWavePulse[type][(((*counter) >> 17) & 0x1f)];
	}
	return 4095* total/2;
#else
	//zhfang
	...
	*counter = temp;
	...
	{
		*counter = *counter + pitch;
		total += g_pWavePulse[type][(((*counter) >> 20) & 0x1f)];
	}
	return 4095* total/16;*/
#endif
...
inline u32 __PitchTo32_VRC_C(u16 pitch)
{
...
inline u32 __PitchTo32_VRC_PULSE(u16 pitch)
{
...
inline u32 __PitchTo32_HVC(u16 pitch)
{
...
inline u32 __SoundSweepRateTo32(u16 rate)
{
	return SOUND_INT_ONE*(OUTPUT_RATE*rate/SOUND_SWEEP_FREQ);
}

inline u32 __SoundEnvelopeRateTo32(u16 rate)
{
