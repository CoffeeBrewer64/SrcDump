...
inline void __ClearSoundEVoltage(void)
{
	delta_counter=0;
	bias = 0;
	bias_move=0;
...
inline void MoveDeltaCounter(u8 input_sample)
{
	if (input_sample)
	{
		//if (delta_counter < 63)
...
inline void WriteBias(u16 nbias)
{
	bias_move = ((s16)nbias-(s16)bias);
	bias = nbias;
}

inline s16 MoveVoltage(void)
{
...
inline u32 __PitchTo32_HVC_E(u16 clock)
{
	return (DMOD_TABLE[clock]);
}


inline void __Sound_Write_E(u16 reg, u8 value)
{
	(void)value;
	switch(reg)
	{
...
inline s16 __ProcessSoundE(void)
{
	s16 sample;
	u16 offset;
	u8 read;
