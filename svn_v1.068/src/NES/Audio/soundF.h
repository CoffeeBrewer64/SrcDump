...
inline u8 __GetWave_Disk(s32 type, s32 counter)
{
#if ( __GETWAVE_DISK_20080112 == 1)
	u8 current = (u8)(SoundF.subwave);
#endif
...
#if ( __GETWAVE_DISK_20080112 == 1)
				if (SoundF.subwave >=96)  SoundF.subwave= (s8)(-32+(SoundF.subwave-96));
...
inline void  __CreateDiskSubWave(void)
{
	u32 i;
	s16 s=0;
...
inline u32 __PitchTo32_DISKFM(u16 pitch)
{
	f32 freq;
	u32 cc;
...
void __Sound_Write_Disk(u16 reg, u8 value)
{
	...
	if ( (reg >= 0x40) && (reg <= 0x7f) )
	{
		...
		//NN_LOG("DISKFM_TABLE[%d] = %x\n", reg - 0x40, ((sbuffer.sr.R_4040[reg-0x40]) & 0x3F));
	}
	else
	{
		switch(reg)
		...
		case 0x89:
			...
			break;
		case 0x8A:
			//osp("Sound: DISKFM Envelope Speed %d\n", sbuffer.sr.R_408A);
...
inline u16 __ProcessSoundF(void)
{
	s8 subsample;
	if (SoundF.sub_writeflag) 
		subsample=0;
	...
	f32 outfreq = ((f32)outvol + 64)/64.0f;
	u16 sample=__GetWave_Disk(DISKFM, SoundF.fcounter.fc.high);
