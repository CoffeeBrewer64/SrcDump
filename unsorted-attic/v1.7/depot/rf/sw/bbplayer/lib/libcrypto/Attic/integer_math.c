...
bigint_mul( bigint *a, bigint *b, bigint *c, int int_max)
{
	...
	short int 	i, j, k;
	bigint	sum;
	bigint_null(c, 4*MAX_MAX_LONG-1);
        bigint_null(&sum, 4*MAX_MAX_LONG-1);
...
bigint_div(bigint *num, bigint *den, bigint *quotient, bigint *remainder, int int_max)
{
    ...
    bigint_null(quotient, 4*MAX_MAX_LONG -1);
    bigint_null(remainder, 4*MAX_MAX_LONG-1);
...
BSL_error 
ascii_to_bigint(char *instring, bigint *outhex, int int_max)
{
    element ch;
    ...
    bigint_null(&ten, int_max);
    ten.half_word[int_max] = 0xA;
    bigint_null(&digit, int_max);
    bigint_null(outhex, int_max);

    while ( ch = *instring++){
        digit.half_word[int_max] = ch & 0xF;
        bigint_mul(outhex, &ten, &temp, int_max);
        if(digit.half_word[int_max] > 9) continue;
        bigint_add(&temp, &digit, outhex, int_max);
    }
    return BSL_OK;
}
...
BSL_error 
bigint_to_ascii(bigint *inhex, char *outstring, int int_max, int size)
{
    ...
    *outstring-- = 0;
    ...
    check = 1;
    while(check){
        bigint_div(&top, &ten, &quotient, &remainder, int_max);
        *outstring-- = remainder.half_word[int_max] | '0';
        check =0;
        for(i=int_max; i>=0; i--){
            check |= quotient.half_word[i];
        }
        bigint_copy(&quotient, &top, int_max);
    }
    return BSL_OK;
}
...
bigint_mod_inv(bigint *a, bigint *b, bigint *x, int int_max)
{
    ...
    element check;
    short int sw, i;
    bigint_null(&m, 4* MAX_MAX_LONG-1);
    bigint_null(&n, 4*MAX_MAX_LONG-1);
    bigint_null(&p0, 4*MAX_MAX_LONG-1);
    bigint_null(&p1, 4*MAX_MAX_LONG-1);
    bigint_null(&p2, 4*MAX_MAX_LONG-1);
    bigint_null(&q, 4*MAX_MAX_LONG-1);
    bigint_null(&r, 4*MAX_MAX_LONG-1);
    bigint_null(&temp, 4*MAX_MAX_LONG-1);
    bigint_null(&dummy, 4*MAX_MAX_LONG-1);
    /* check errors */
...
field_to_bigint(field_2n *a, bigint *b, int num_bits)
{
  ...
  bigint_null(b, 4*MAX_MAX_LONG -1);
...
/* utilities for conversion from compact word format to native 
 * bigint format. bigint to certificate format word representation 
 */

void 
bigint_to_certint(unsigned long *certfmt, int certfmtwords, bigint *b, int int_max){
    int i;
    int index = certfmtwords -1;
    for(i = int_max; i >=0; i= i - 2){
        if(index >=0){
            certfmt[index] = (b->half_word[i]&0xffff) |((b->half_word[i-1] &0xffff) << 16);
            index--;
        }
    }
}
...
void
certint_to_bigint( bigint *b, int int_max, unsigned long *certfmt, int certfmtwords){
    int i;
    int index = int_max;
    bigint_null(b, int_max);
    for(i = certfmtwords-1; i >=0; i--){
        b->half_word[index] = certfmt[i] & 0xffff;
        b->half_word[index - 1] = (certfmt[i] & 0xffff0000) >> 16;
        index = index -2;
    }
}
 ...
 * takes words from publickey and sign and exponent in certificate format
 * converts to native bigint and computes byte string of expected hash
 */
BSL_error 
bsl_rsa_verify(char *result, unsigned long *certsign, unsigned long *certpublickey, unsigned long *certexponent,  int num_bits){
    ...
    max_long = num_word + 1;
    int_max = (4*max_long) - 1;
    certint_to_bigint(&m, int_max, certsign, num_word);
    certint_to_bigint(&n, int_max, certpublickey, num_word);
    certint_to_bigint(&e, int_max, certexponent, 1 );
        
    /* compute o = m^e mod n */
    err = mod_exp(&m, &e, &n, &o, int_max);
    
    /* go back to string*/
    bigint_to_charstring(result, num_bits/8 , &o, int_max);
    return  err;
}
#if 0
