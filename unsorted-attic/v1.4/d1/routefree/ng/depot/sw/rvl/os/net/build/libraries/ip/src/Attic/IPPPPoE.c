/*---------------------------------------------------------------------------*
  ...
  $Log: IPPPPoE.c,v $
  Revision 1.3  2006/04/18 17:46:24  mxu
  add processing for NET_IP_ALIGN
...
#ifdef IOP_OS
         tag = (PPPoETag *) ((u8 *) tag + PPPoE_TAGLEN(tag)))
#else
...
#ifdef IOP_OS
    PPPoEHeader* pppoe = (PPPoEHeader*) ((u8*) eh + ETH_HLEN + NET_IP_ALIGN);
#else
...
#ifdef IOP_OS
    // Check length
    if (len < ETH_HLEN + NET_IP_ALIGN + PPPoE_HLEN ||
        len < ETH_HLEN + NET_IP_ALIGN + PPPoE_HLEN + pppoe->len)
#else
