/*---------------------------------------------------------------------------*
  ...
  initial network stack code base
    ...
    53    4/03/06 19:21 Shiki
    Modified IP6GetPathMTU() not to increase PMTU less than 1 min after the
    previous attempted increase.

    52    3/20/06 10:15 Shiki
    Fixed UDP4Out() and UDP6Out() so that they can send UPD datagram whose
    data length is zero.
                      ...
                      UDPCallback callback, s32* result)
{
    ...
    else if (obj->sendUsed <= 0)
    {
        ...
        obj->sendUsed = IP_MIN_HLEN + UDP_HLEN + len;
                   ...
                   UDPCallback callback, s32* result)
{
    ...
    else if (obj->sendUsed <= 0)
    {
        ...
        memmove(obj->sendData, data, (u32) len);
        obj->sendUsed = (long) sizeof(IP6Hdr) + UDP_HLEN + len;
    ...
    datagram->vec[1].len  = len;
    datagram->param = info;
    datagram->mtu = (u16) IP6GetPathMTU(destination, datagram->vec[0].len + len);
