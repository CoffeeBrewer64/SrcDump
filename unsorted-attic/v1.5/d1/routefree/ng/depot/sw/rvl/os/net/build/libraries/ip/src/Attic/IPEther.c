/*---------------------------------------------------------------------------*
  ...
  $Log: IPEther.c,v $
  Revision 1.4  2006/04/18 01:59:19  mxu
  add the processing of NET_IP_ALIGN which makes headers aligned on 4-byte boundary

  Revision 1.3  2006/04/08 01:13:14  mxu
...
void ETHIn(IPInterface* interface, ETHHeader* eh, s32 len)
{
    ...
    switch (eh->type)
    {
...
#ifdef IOP_OS
        ip = (IPHeader*) ((u8*) eh + ETH_HLEN + NET_IP_ALIGN);
#else
...
#ifdef VERBOSE
#ifdef IOP_OS
        PPPoEDumpPacket((PPPoEHeader*) ((u8*) eh + ETH_HLEN + NET_IP_ALIGN));
#else
      ...
      case ETH_IPv6:
        ...
        //
#ifdef IOP_OS
        ip6 = (IP6Hdr*) ((u8*) eh + ETH_HLEN + NET_IP_ALIGN);
#else
...
#ifdef IOP_OS
        IP6In(interface, ip6, len - ETH_HLEN - NET_IP_ALIGN, flag);
#else
...
static BOOL Go(void)
{
    ...
    {
        OSDbgReport(("%s: loopback: interface->loopback=%d, interface->sending=%d\n", 
            __FUNCTION__, interface->loopback, interface->sending));
