/*---------------------------------------------------------------------------*
  ...
  $Log: IPTcp.c,v $
  Revision 1.4  2006/04/14 22:59:19  mxu
  ...
  initial network stack code base
    ...
    88    4/04/06 11:29 Shiki
    Clean up.

    87    4/04/06 9:10 Shiki
    Fixed DoOption().

    86    4/03/06 19:21 Shiki
    Modified IP6GetPathMTU() not to increase PMTU less than 1 min after the
    previous attempted increase.
    Modified SO_TCP_MAXSEG management code.
...
static TCPObj* DoListen(IPInterface* interface, TCPObj* info, void* ip, TCPHeader* seg, s32 len)
{
    ...
    accepted->userBuff = 0;
    accepted->userLen = 0;
    accepted->mssBase = info->mssBase;
    accepted->mss = info->mss;
    ...
    switch (info->pair->family)
    {
      case SO_PF_INET:
        ...
        mss = (u16) (interface->mtu - IP_MIN_HLEN - TCP_MIN_HLEN);
        if (accepted->mssBase == 0)
        {
            accepted->mssBase = mss;
        }
        else
        {
            accepted->mssBase = MIN(accepted->mssBase, mss);
        }
        accepted->mss = accepted->mssBase;
      ...
      case SO_PF_INET6:
        ...
        mss = (u16) (interface->linkMTU - sizeof(IP6Hdr) - TCP_MIN_HLEN);
        if (accepted->mssBase == 0)
        {
            accepted->mssBase = mss;
        }
