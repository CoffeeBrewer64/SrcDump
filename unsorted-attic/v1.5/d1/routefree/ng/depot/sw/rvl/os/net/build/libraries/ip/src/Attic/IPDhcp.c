/*---------------------------------------------------------------------------*
  ...
  $Log: IPDhcp.c,v $
  Revision 1.4  2006/04/12 02:06:03  mxu
  change strlen to strnlen
  ...
  initial network stack code base
    ...
    36    3/30/06 14:14 Shiki
    Fixed to not to set the limited broadcast address to the interface by
    default.
...
static void InitDHCPInfo(DHCPInfo* info)
{
    ...
    info->ttl = IP_TTL;
    info->mtu = 0;
    memmove(info->broadcast, IPAddrAny, IP_ALEN);
...
static void UpdateDHCPInfo(DHCPInfo* info, DHCPInfo* update)
{
    ...
    if (IP_ADDR_NE(update->broadcast, IPAddrAny))
    {
...
#ifdef IOP_OS
        len = strnlen(ctrl->hostName, DNS_NAME_MAX + 1);
#else
...
static void Start(DHCPControl* ctrl)
{
    ...
    if (IP_ADDR_NE(info->broadcast, IPAddrAny) &&
        IPIsBroadcastAddr(&__IFDefault, info->broadcast))
    {
...
int DHCPGetOpt(int opt, void* buf, int len)
{
    ...
    switch (opt)
    {
      ...
      case DHCP_OPT_HOST_NAME:
#ifdef IOP_OS
        ulen = MIN(strnlen(ctrl->info.host, DNS_NAME_MAX + 1), ulen);
#else
      ...
      case DHCP_OPT_DOMAIN_NAME:
#ifdef IOP_OS
        ulen = MIN(strnlen(ctrl->info.domain, DNS_NAME_MAX + 1), ulen);
#else
