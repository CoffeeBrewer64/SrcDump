/*---------------------------------------------------------------------------*
  ...
  initial network stack code base
    ...
    84    4/04/06 15:47 Shiki
    Clean up.

    83    3/31/06 11:50 Shiki
    Modified LingerTimeout() to set IP_ERR_TIMEOUT to TCPObj.err.
    Modified __SOClose() to wait up to R2 when linger off.

    82    06/03/29 16:04 Ueno_kyu
    Modified SOStartup() to clear __SOResolver.info.maxQuery and
    __SOResolver.info.port.

    81    3/09/06 14:04 Shiki
    Fixed SOInetAtoN() and SOInetPtoN() so they can handle host names whose
    first character is a digit.

    80    3/08/06 17:05 Shiki
    Modified SOStartup() to keep __SOResolver.info.maxQuery and
    __SOResolver.info.port.
...
int SOInetAtoN(const char* cp, SOInAddr* inp)
{
    ...
    cp = IPAtoN(cp, inp ? (u8*) &inp->addr : addr);
    return (cp && !(isalnum)(*cp)) ? 1 : 0;
}
...
int SOSetSuffix(const char* domainName)
{
    ...
    {
#ifdef IOP_OS
        strncpy(__SOResolver.suffix, domainName, strnlen(domainName, SO_MAXDNAME));
#else
...
int SOGetSuffix(char* domainName)
{
    ...
    {
#ifdef IOP_OS
        strncpy(domainName, __SOResolver.suffix, strnlen(domainName, SO_MAXDNAME));
#else
    ...
    switch (info->proto)
    {
      ...
      case IP_PROTO_TCP:
        ...
        else
        {
            ...
            // in background and waits up to R2 seconds.
            OSSetAlarm(&obj->lingerAlarm, obj->r2, LingerTimeout);
