...
static void 
run_prog(IOSEntryProc prog_sym, u32 pri)
{
    u32 *progStack = (void *) malloc(EMU_STACK_SIZE);
    u32 id = iosCreateThread(prog_sym, 0, progStack, EMU_STACK_SIZE, pri);
    iosStartThread(id);
    return;
}

void 
iosEmuStartProg(const char *prog, u32 pri)
{
  char slib[SDIR_MAX];
  char *sdkroot = NULL;
  IOSEntryProc prog_sym;
  void *handle = NULL;
  char *error;
  int shell = -1;

  if ( 0 != (shell = strcmp(prog, IOSSH_PROG_NAME) ))
      sdkroot = getenv("ROOT");
