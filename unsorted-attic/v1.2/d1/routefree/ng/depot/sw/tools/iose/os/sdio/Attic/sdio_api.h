...
/*
 * $Id: sdio_api.h,v 1.1 2005/09/26 22:39:24 jlivesey Exp $
 */
...
typedef void (*SDCallbackFunc)(int);
...
typedef enum {
    SDDevIllegal,   /* illegal thread type, error is lower down */
    SDDevRemove,
    SDDDevInterrupt,
    SDDevError,
} SDDevThread;

typedef struct _SDDevHandle {
    int  SDDevFd;
    u8   SDDevSlot;
    u8   SDDevFunc;
    char *SDDevName;
    u16  SDDevRca;
    SDCallbackFunc SDDevInterruptHandler;
    SDCallbackFunc SDDevRemoveHandler;
    SDCallbackFunc SDDevErrorHandler;
    pthread_t  SDDevInt_thread; 
    pthread_t  SDDevErr_thread; 
    pthread_t  SDDevRemoval_thread; 
    int  SDDevInt_tid; 
    int  SDDevErr_tid; 
    int  SDDevRemoval_tid; 
} *SDDevHandle;
 ...
 * error notification.
 */
extern IOSError ISD_ReadReg(SDDevHandle fd, u32 addr, u32 size, u32 *data);
extern IOSError ISD_WriteReg(SDDevHandle fd, u32 addr, u32 size, u32 data);
...
extern IOSError ISD_ReadMultiReg(SDDevHandle fd, u32 addr, 
                                 u32 count, u32 incrFlag, u32 *data);
extern IOSError ISD_WriteMultiReg(SDDevHandle fd, u32 addr, 
                                  u32 count, u32 incrFlag, u32 *data);


/*
 * Non-public internal routines.
 */
extern IOSError sdio_thread_create (SDDevHandle fd, SDDevThread type);
