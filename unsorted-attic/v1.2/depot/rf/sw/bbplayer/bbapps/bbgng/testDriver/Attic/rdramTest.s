/*******************************************************************
  改訂履歴 和田
  96/10  実際の検査データの書き込み／ベリファイ部分を１６回ループ展開＆
	 遅延スロットをnopから標準命令に変更し、分岐後に
	 そのペナルティを支払う形に変更
  *****************************************************************/
...
write_loop:
	sw	k0,0(k0)		# write address pattern 0
	addiu	k0,4
	sw	k0,0(k0)		# write address pattern 1
	addiu	k0,4
	sw	k0,0(k0)		# write address pattern 2
	addiu	k0,4
	sw	k0,0(k0)		# write address pattern 3
	addiu	k0,4
	sw	k0,0(k0)		# write address pattern 4
	addiu	k0,4
	sw	k0,0(k0)		# write address pattern 5
	addiu	k0,4
	sw	k0,0(k0)		# write address pattern 6
	addiu	k0,4
	sw	k0,0(k0)		# write address pattern 7
	addiu	k0,4 
	sw	k0,0(k0)		# write address pattern 8
	addiu	k0,4
	sw	k0,0(k0)		# write address pattern 9
	addiu	k0,4
	sw	k0,0(k0)		# write address pattern A
	addiu	k0,4
	sw	k0,0(k0)		# write address pattern B
	addiu	k0,4
	sw	k0,0(k0)		# write address pattern C
	addiu	k0,4
	sw	k0,0(k0)		# write address pattern D
	addiu	k0,4
	sw	k0,0(k0)		# write address pattern E
	addiu	k0,4
	sw	k0,0(k0)		# write address pattern F
	addiu	k0,4
...
/* sasano's trap end */
	...
	li	k0,start_address
check_loop:
	lw	t0,0(k0)		# write address pattern 0
 #debug	addiu	t1, k0, 0		# store expected value in t1
	bne	t0,k0,fail1
 #debug	nop
	addiu	k0,4			#ここは遅延スロットです

	lw	t0,0(k0)		# write address pattern 1
	bne	t0,k0,fail1
	addiu	k0,4			#ここは遅延スロットです
	lw	t0,0(k0)		# write address pattern 2
	bne	t0,k0,fail1
	addiu	k0,4			#ここは遅延スロットです
	lw	t0,0(k0)		# write address pattern 3
	bne	t0,k0,fail1
	addiu	k0,4			#ここは遅延スロットです
	lw	t0,0(k0)		# write address pattern 4
	bne	t0,k0,fail1
	addiu	k0,4			#ここは遅延スロットです 
	lw	t0,0(k0)		# write address pattern 5
	bne	t0,k0,fail1
	addiu	k0,4			#ここは遅延スロットです
	lw	t0,0(k0)		# write address pattern 6
	bne	t0,k0,fail1
	addiu	k0,4			#ここは遅延スロットです
	lw	t0,0(k0)		# write address pattern 7
	bne	t0,k0,fail1
	addiu	k0,4			#ここは遅延スロットです
	lw	t0,0(k0)		# write address pattern 8
	bne	t0,k0,fail1
	addiu	k0,4			#ここは遅延スロットです
	lw	t0,0(k0)		# write address pattern 9
	bne	t0,k0,fail1
	addiu	k0,4			#ここは遅延スロットです
	lw	t0,0(k0)		# write address pattern A
	bne	t0,k0,fail1
	addiu	k0,4			#ここは遅延スロットです
	lw	t0,0(k0)		# write address pattern B
	bne	t0,k0,fail1
	addiu	k0,4			#ここは遅延スロットです
	lw	t0,0(k0)		# write address pattern C
	bne	t0,k0,fail1
	addiu	k0,4			#ここは遅延スロットです
	lw	t0,0(k0)		# write address pattern D
	bne	t0,k0,fail1
	addiu	k0,4			#ここは遅延スロットです
	lw	t0,0(k0)		# write address pattern E
	bne	t0,k0,fail1
	addiu	k0,4			#ここは遅延スロットです
	lw	t0,0(k0)		# write address pattern F
	bne	t0,k0,fail1
	addiu	k0,4			#ここは遅延スロットです
...
write_loop1:
	xor	t1,k0,t3
	sw	t1,0(k0)		# write address pattern 0
	addiu	k0,4
	xor	t1,k0,t3
	sw	t1,0(k0)		# write address pattern 1
	addiu	k0,4
	xor	t1,k0,t3
	sw	t1,0(k0)		# write address pattern 2
	addiu	k0,4
	xor	t1,k0,t3
	sw	t1,0(k0)		# write address pattern 3
	addiu	k0,4
	xor	t1,k0,t3
	sw	t1,0(k0)		# write address pattern 4
	addiu	k0,4
	xor	t1,k0,t3
	sw	t1,0(k0)		# write address pattern 5
	addiu	k0,4
	xor	t1,k0,t3
	sw	t1,0(k0)		# write address pattern 6
	addiu	k0,4
	xor	t1,k0,t3
	sw	t1,0(k0)		# write address pattern 7
	addiu	k0,4
	xor	t1,k0,t3
	sw	t1,0(k0)		# write address pattern 8
	addiu	k0,4
	xor	t1,k0,t3
	sw	t1,0(k0)		# write address pattern 9
	addiu	k0,4
	xor	t1,k0,t3
	sw	t1,0(k0)		# write address pattern A
	addiu	k0,4
	xor	t1,k0,t3
	sw	t1,0(k0)		# write address pattern B
	addiu	k0,4
	xor	t1,k0,t3
	sw	t1,0(k0)		# write address pattern C
	addiu	k0,4
	xor	t1,k0,t3
	sw	t1,0(k0)		# write address pattern D
	addiu	k0,4
	xor	t1,k0,t3
	sw	t1,0(k0)		# write address pattern E
	addiu	k0,4
	xor	t1,k0,t3
	sw	t1,0(k0)		# write address pattern F
	addiu	k0,4
...
check_loop1:
	lw	t0,0(k0)		# write address pattern 0
	xor	t1,k0,t3
	bne	t0,t1,fail2		# t1 has expected value
 #debug	nop	
	addiu	k0,4                    #注意:ここは遅延スロットです
	...
	sw	zero, rdram_diag_status_expect(t0)
	sw	zero, rdram_diag_status_readdt(t0)
	sw	zero, rdram_diag_status_sasano(t0)
	b	load_app		# now load app/os...
	nop

fail1:
	addiu	k0,-4			#遅延スロットでひいちゃった分を戻す
	li	t3, reserved_mem
	sw	k0,  rdram_diag_status_expect(t3)
	sw	t0, rdram_diag_status_readdt(t3)
	li	t1, 1
	sw	t1, rdram_diag_errors_offset(t3)
	b	fail

fail2:
	addiu	k0,-4			#遅延スロットでひいちゃった分を戻す
	li	t3, reserved_mem
	sw	t1, rdram_diag_status_expect(t3)
	sw	t0, rdram_diag_status_readdt(t3)
	li	t1, 2
	sw	t1, rdram_diag_errors_offset(t3)
...
fail:
	...
	li	t1, 0xdeadbeef
	sw	t1, rdram_diag_status_offset(t0)	
  sw  k0, rdram_diag_status_sasano(t0)
	/* 
